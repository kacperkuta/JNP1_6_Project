...
bool File::registerStrat(std::string name, StratCreateMethod strat) {
    strats()[name] = strat;
    return true;
}
...
std::shared_ptr<OpeningStrategy> OpenSong::createStrat() {
    return std::dynamic_pointer_cast<OpeningStrategy>(
            std::make_shared<OpenSong>());
}
